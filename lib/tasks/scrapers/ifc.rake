namespace :db do
  namespace :scrape do
    ...
    task ifc: :environment do
      ...
      def scrape_ifc_doc(ifc_doc_url, ifc_id, ifc_url)
        ifc_doc = Nokogiri::HTML(open(ifc_doc_url))
        ...
        a_nodes = ifc_doc.css('.coming_soon_pagination.pagination-bottom a')
        next_node = a_nodes.select do |a|
          a.content['Next']
        end

        if next_node.present?
          next_url = ifc_url.chop + next_node.first.attributes['href'].value
          scrape_ifc_doc(next_url, ifc_id, ifc_url)
        end
      end
      ...
      scrape_ifc_doc(ifc_coming_soon_url, ifc_id, ifc_url)
    end
  end
end
